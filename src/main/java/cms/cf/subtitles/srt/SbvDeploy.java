...
public class SbvDeploy
{
    ...
    {
        //main1(new String[]{"F:\\Caio Fabio - projetos\\Subtitles\\ZHLJSplQi_c.sbv"});
        main1(new String[]{"F:\\Caio Fabio - projetos\\Subtitles\\U9qEgMIBQ3M.sbv"}); 
    }
    ...
    public static void main1(String[] args)
    {
        if (args.length < 1)
        {
            System.err.println("Arqumentos invalidos. Formato Correto:\n        java cms.cf.subtitles.srt.SbvDeploy 'path to SBV file'");
            System.exit(1);
        }

        File file = new File(args[0]);
        String name = file.getName();
        if (name.endsWith(".sbv") == false)
        {
            System.err.println("Argumento precisa ser um arquivo '.sbv'");
            System.exit(1);
        }
        
        String idYoutube = name.substring(0,name.indexOf(".sbv"));
        ...
        try
        {
            ...
            Srt srt = Srt.readSbvFile(file, "UTF-8");
